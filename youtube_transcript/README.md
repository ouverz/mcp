## How to Automate Youtube Video Transcript Retrieval Using MCP

This tutorial shows how to build a Youtube video transcript retrieval agent using Model Context Protocol (MCP) server in Python, managed by `uv`.
In this case we retrieve the transcript using the Youtube Transcript API that is available.
...
Restart Claude Desktop. You should see tools like `get_transcript` available and more importantly 'online'. If it has a 'failed' status something went wrong,
please revert back to previous steps and make sure you have followed them and completed them successfully.
...
# reference to: https://github.com/daveebbelaar/ai-cookbook/blob/main/mcp/servers/youtube/README.md
